...
class Trade:
    ...
    def get_amount(self):  # TODO rename ***
        self.payment_amount = Trade.format_amount(self.payment_method.token, self.payment_amount)
    ...
    def inverse(self):
        ...
        if self.withdraw_method.is_fiat:
            withdraw_amount = Trade.format_amount_up(self.withdraw_method.token, self.withdraw_amount)  # FIXME
            if self.p2p_item_buy is None:
                print('get p2p')
            ...
            usdt_amount = Trade.format_amount_up('USDT', usdt_amount)
            print('usdt_amount', usdt_amount)
        ...
        if self.payment_method.is_fiat:
            ...
            payment_amount = Trade.format_amount_up(self.payment_method.token, payment_amount)  # FIXME
            ...
            payment_amount = Trade.format_amount_up(self.payment_method.token, payment_amount)
        ...
        if best_p2p is None:
            if ((p2p_side == P2PItem.SIDE_SELL and payment_amount == 0.0 and withdraw_amount != 0.0) or
                ...
                if not items_query.annotate(req_amount=(F('price') * (withdraw_amount or payment_amount))).filter(Q(req_amount__gt=F('min_amount'))).exists():
                    ...
                    raise MinWithdrawException(min_amount)
                ...
                if not items_query.annotate(req_amount=(F('price') * (withdraw_amount or payment_amount))).filter(Q(req_amount__lt=F('max_amount'))).exists():
                    ...
                    raise MaxWithdrawException(max_amount)
