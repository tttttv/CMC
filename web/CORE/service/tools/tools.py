...
def calculate_withdraw_quantity(token, chain, amount, p2p_price, withdraw_token_rate, platform_commission,
                                partner_commission, trading_commission, chain_commission):
    digits = TOKENS_DIGITS[token]
    return format_float(
        (((amount / p2p_price) * (1 - partner_commission - platform_commission)) / withdraw_token_rate) * (
                    1 - trading_commission) - chain_commission
        , digits)
...
def get_price(payment_method, amount, quantity, currency, token, chain, platform_commission, partner_commission,
    ...
    if amount == 0 and quantity != 0:
        all_items = P2PItem.objects.filter(side=P2PItem.SIDE_SELL, is_active=True, currency=currency,
                                           token=p2p_token).order_by('-price').all()
        items = []
        for item in all_items:
            print(item.price * quantity, item.min_amount, item.max_amount)
            if item.min_amount < item.price * quantity < item.max_amount:
                items.append(item)
    else:
        items = P2PItem.objects.filter(side=P2PItem.SIDE_SELL, is_active=True, min_amount__lte=amount,
                                       max_amount__gte=amount, currency=currency, token=p2p_token).order_by(
            '-price').all()
    ...
    better_items = P2PItem.objects.filter(side=P2PItem.SIDE_SELL, is_active=True, max_amount__gte=amount,
                                          currency=currency, token=p2p_token, price__lt=best_p2p.price) \
        .order_by('min_amount', 'price').all()
    ...
    if anchor == P2POrderBuyToken.ANCHOR_CURRENCY:  # Считаем сколько крипты получит клиент
        quantity = calculate_withdraw_quantity(token, chain, amount, p2p_price, trade_rate, platform_commission,
                                               partner_commission, trading_commission, chain_commission)
    elif anchor == P2POrderBuyToken.ANCHOR_TOKEN:  # Считаем сколько нужно заплатить фиата за количество крипты
        amount = calculate_topup_amount(token, quantity, p2p_price, trade_rate, platform_commission, partner_commission,
                                        trading_commission, chain_commission)
